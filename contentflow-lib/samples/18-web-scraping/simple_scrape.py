"""
Simple web scraping example using the WebScrapingExecutor.
...
async def main():
    ...
    # Load configuration
    config_path = Path(__file__).parent / "simple_scrape.yaml"
    ...
    output_file = output_folder / "simple_scraping_result.json"
    with open(output_file, 'w', encoding='utf-8') as f:
            [f.write(result.model_dump_json(indent=2)) for result in results]
